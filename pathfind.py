...
class ReservationTable:
    ...
    def check_edge_capacity(u: str, v: str) -> int:
    ...
    def is_edge_free(u: str, v: str, turn: int) -> bool:
        cap = check_edge_capacity(u, v)
        current = self.edge_traffic.get((self.edge_key(u, v), turn), 0)
        return current < cap
    
    def is_node_free(z_name: str, turn: int) -> bool:
        if z_name in (self.graph.start_hub.name, self.graph.end_hub.name):
            return True
        
        limit = self.graph.zones[zone_name].nb_drones
        current = self.node_traffic.get((zone_name, turn), 0)
        return limit > current
    
    def book_node(z_name: str, turn: int) -> None:
        if z_name not in (self.graph.start_hub.name, self.graph.end_hub.name):
            key = (zone_name, turn)
            self.node_traffic[key] = self.node_traffic.get(key, 0) + 1
    
    def book_edge(u: str, v: str, turn: int) -> None:
...
class Pathfinder:
    ...
    def k_paths(k: int = 5) -> List[list[str]]:
        ...
        for _ in range(k):
            path = _djikstra(start, end, edge_penalties)
            ...
            for i in range(len(path) - 1):
                ...
                edge_penalties[path] = edge_penalties.get(pair, 0) + 5

        return paths

    def _djikstra(start: str, end: str, penalties: Dict) -> List[str]:
        pq = ([0, start, [start]])
        best_costs = {start: 0}
        ...
        while True:
            ...
            for conn in self.graph.connections.get(current, []):
    ...
    def _simulate_path(self, drone_id: int, path: List[str]):
        ...
        for i in range(len(path) - 1):
            ...
            while True:
                ...
                edge_free = self.table.is_edge_free(curr_room
                                        , next_room, curr_turn)
                ...
                reservations.append((curr_room, next_room, curr_turn, arrival_turn))
