...
class Zone(BaseModel):
    ...
    def validate_name(cls, v: str) -> str:
        if " " in v or "-" in v:
            raise ValueError("Name must not have a space or a '-'.")
        return v
...
class Connection(BaseModel):
    ...
    @classmethod
    def prevent_self_loop(cls, v: str, info: ValidationInfo) -> str:
...
class Graph(BaseModel):
    ...
    def connect_zones(self, conn: Connection) -> None:
        if (
            conn.zone1_name not in self.zones
            or conn.zone2_name not in self.zones
        ):
            raise ValueError(
                "Connection links undefined zone: "
                f"{conn.zone1_name} or {conn.zone2_name}."
            )
